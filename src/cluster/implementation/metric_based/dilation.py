...
class MetricBasedDilator(DilationProtocol[Kernel, State]):
    ...
    def __init__(
        ...
        self.update(state)
        assert self._n_levels > 1, "Dilation can't be called on two small values" # TODO: Improve in the future

    def _zoom_out(self) -> DilationOperationReturnType:
        if self._current_dilation_level_ptr == self._n_levels -1:
            raise ValueError("Already at top level, cannot zoom out")
        ...
        current_window = get_window_from_cell(self._dilation_levels, self._current_dilation_level_ptr, selected_cell,self._kernel)
    ...
    def _zoom_in(self, selected_cell: tp.Tuple[int, int]) -> DilationOperationReturnType:
        current_window = get_window_from_cell(
            self._dilation_levels,
            self._current_dilation_level_ptr,
            selected_cell,
            self._kernel
        )
    ...
    def _select_real_machine(self, selected_cell: tp.Tuple[int, int]) -> DilationOperationReturnType:
        assert self._current_dilation_level_ptr == 1, f"When running _select_real_machine level pointer should be equal to 1 and not {self._current_dilation_level_ptr}"

        self._current_dilation_level_ptr -= 1
        self._prev_selected_cell[self._current_dilation_level_ptr] = selected_cell

        return DilationOperationReturnType(DilationOperation.Execute, None)
    ...
    def update(self, original: State) -> State:
        self._dilation_levels = hierarchical_pooling(original, self._kernel, fill_value=self._fill_value)
