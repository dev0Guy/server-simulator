...
from .base import AbstractClusterGameRenderer
from src.envs.utils.observation_extractors.metric_observation_extractor import (
    MetricClusterObservation,
)
from src.envs.utils.info_builders.base import ClusterBaseInformation
...
class ClusterMetricRenderer(
    ...
    def _draw_jobs(
        ...
        # Sort jobs by status
        sorted_indices = self._sort_jobs_by_status(jobs_status)
    ...
    def _sort_jobs_by_status(self, jobs_status: npt.NDArray) -> np.ndarray:
        """
        Sort job indices by status priority.

        Order: Running -> Pending -> Failed -> Completed -> NotCreated

        Args:
            jobs_status: Array of Status enum values

        Returns:
            Array of sorted job indices
        """
        n_jobs = len(jobs_status)

        # Create array of (priority, original_index) tuples
        priorities = np.array(
            [self.status_order.get(jobs_status[i], 999) for i in range(n_jobs)]
        )

        # Sort by priority and return indices
