...
from src.cluster.implementation.single_slot import SingleSlotCluster, SingleSlotMachinesConvertor, SingleSlotJobsConvertor
import numpy as np
...
class SingleSlotObservationCreator(BaseObservationCreatorProtocol[SingleSlotCluster, SingleSlotClusterObservation]):
    ...
    def create(self, cluster: SingleSlotCluster) -> SingleSlotClusterObservation:
        ...
        return SingleSlotClusterObservation(
            ...
            jobs_status=np.array([status.value for status in job_status], dtype=np.float64),
