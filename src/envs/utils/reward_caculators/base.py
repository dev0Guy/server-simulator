...
class DifferentInPendingJobsRewardCaculator(RewardCaculator[ClusterInformation]):
    def __call__(
        ...
        prev_not_pending_jobs_count = sum(s != Status.Pending for s in prev_extra_information["jobs_status"])
        current_not_pending_jobs_count = sum(s != Status.Pending for s in current_extra_information["jobs_status"])
