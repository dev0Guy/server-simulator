...
array_strategy = st.builds(
    lambda m_x, m_y, n_resources, n_ticks: np.random.rand(
        m_x, m_y, n_resources, n_ticks).astype(np.float64),
    m_x=st.integers(2, 8),
    m_y=st.integers(2, 8),
...
def dilator_kernel_and_points(draw):
    ...
    points = draw(points_strategy(
        kernel[0]-1, kernel[1]-1, min_size=min_points))
...
def test_dilator_init(array, kernel, operation):
    ...
    assert dilator._n_levels == len(
        dilator._dilation_levels), "Number of levels mismatch"
    ...
    match dilator.state:
        ...
        case _:
            raise AssertionError(
                f"Dilator initialize step value is not `DilationSteps.Initial`")
...
def test_expand_and_contract(array, kernel_with_points, operation):
    ...
    match expanded_state:
        case DilationState.Expanded(_, _, value, level) | DilationState.FullyExpanded(_, _, value, level):
...
def test_zoom_in_until_fully_expanded(array, kernel_with_points, operation):
    ...
    for cell in expand_cells:
        match dilator.expand(cell):
            case DilationState.Expanded(_, _, value, level):
...
def test_calculate_original_machine_index(rows, cols, x, y):
    ...
    idx = MetricBasedDilator._calculate_original_machine_index(
        cell, original_2d_shape)
