...
from src.cluster.implementation.single_slot.jobs import SingleSlotJobsConvertor
from src.cluster.implementation.single_slot.machines import SingleSlotMachinesConvertor
...
def test_float_cluster_creation(n_machines: int, n_jobs: int, jobs_convertor: SingleSlotJobsConvertor, machine_convertor: SingleSlotMachinesConvertor):
    ...
    assert np.all(jobs_usage >= 0.0) and np.all(jobs_usage <= 1.0), "All cell value should be in range [0,1]"
...
def test_reproducibility(n_machines: int, n_jobs: int, seed: int,  jobs_convertor: SingleSlotJobsConvertor):
    ...
    assert all(status1 == status2 for status1, status2 in zip(jobs_1_status, jobs_2_status))


@pytest.mark.usefixtures("jobs_convertor", "machine_convertor")
@given(n_machines=machines_strategy, n_jobs=jobs_strategy)
@settings(max_examples=30)
def test_schedule_available_last_job_to_first_machine(n_machines: int, n_jobs: int, jobs_convertor: SingleSlotJobsConvertor, machine_convertor: SingleSlotMachinesConvertor) -> None:
    m_idx, j_idx = (0, n_jobs-1)

    cluster = random_machine_with_static_machine(n_machines, n_jobs)
    before_jobs_usage, before_jobs_status = jobs_convertor.to_representation(cluster._jobs)
    ...
    after_jobs_usage, after_jobs_status = jobs_convertor.to_representation(cluster._jobs)
...
@given(n_machines=machines_strategy, n_jobs=jobs_strategy)
@settings(max_examples=30)
def test_schedule_twice_the_same(n_machines: int, n_jobs: int, jobs_convertor: SingleSlotJobsConvertor, machine_convertor: SingleSlotMachinesConvertor) -> None:
    m_idx, j_idx = (0, n_jobs - 1)
    cluster = random_machine_with_static_machine(n_machines, n_jobs)
    ...
    assert np.equal(
        ...
        before_machines[m_idx] -
        before_jobs_usage[j_idx],
    )
    ...
    after_second_jobs_usage, after_second_jobs_status = jobs_convertor.to_representation(cluster._jobs)
    assert np.all(
        after_machines
        == after_second_machines
    ), "UnSchedule action change machine state, which its shouldn't"
    assert np.all(
        after_jobs_usage == after_second_jobs_usage
...
@given(n_machines=machines_strategy, n_jobs=jobs_strategy)
@settings(max_examples=30)
def test_schedule_and_tick(n_machines: int, n_jobs: int,  jobs_convertor: SingleSlotJobsConvertor, machine_convertor: SingleSlotMachinesConvertor) -> None:
    m_idx, j_idx = (0, n_jobs - 1)
    cluster = random_machine_with_static_machine(n_machines, n_jobs)
    ...
    after_schedule_jobs_usage, after_schedule_jobs_status = jobs_convertor.to_representation(cluster._jobs)
    assert np.equal(
    ...
    after_tick_jobs_usage, after_tick_jobs_status = jobs_convertor.to_representation(cluster._jobs)
    assert after_tick_jobs_status[j_idx] == JobStatus.Completed
    assert np.all(
        after_tick_machines_usage == initialize_machines_usage
...
@given(n_machines=machines_strategy, n_jobs=jobs_strategy)
@settings(max_examples=30)
def test_tick_without_schedule(n_machines: int, n_jobs: int, jobs_convertor: SingleSlotJobsConvertor, machine_convertor: SingleSlotMachinesConvertor) -> None:
    cluster = random_machine_with_static_machine(n_machines, n_jobs)
    initialize_machines_usage = machine_convertor.to_representation(cluster._machines)
...
def test_single_machine_multiple_one_size_jobs(n_jobs: int, n_machines: int, jobs_convertor: SingleSlotJobsConvertor, machine_convertor: SingleSlotMachinesConvertor):
...
@settings(max_examples=20)
def test_single_machine_multiple_half_size_jobs(n_jobs: int, n_machines: int,  jobs_convertor: SingleSlotJobsConvertor, machine_convertor: SingleSlotMachinesConvertor):
    static_value = 0.5
    ...
    for j_idx in range(0, cluster.n_jobs, 2):
        ...
        after_second_schedule_jobs_usage, after_second_schedule_jobs_status = jobs_convertor.to_representation(cluster._jobs)
        after_second_schedule_machines_usage = machine_convertor.to_representation(cluster._machines)
