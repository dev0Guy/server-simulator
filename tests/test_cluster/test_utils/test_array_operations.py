...
array_strategy = st.builds(
    lambda m_x, m_y, n_res, n_ticks: np.random.rand(
        m_x, m_y, n_res, n_ticks).astype(np.float64),
...
reduction_operation_strategy = st.sampled_from(
    [np.mean, np.max, np.min, np.average])
...
def block_operation(array, kernel, operation):
    ...
    reshaped = array[:n_x*k_x, :n_y *
                     k_y].reshape(n_x, k_x, n_y, k_y, *array.shape[2:])
...
def test_value_error_when_array_is_not_divisible_by_kernel(arr: npt.NDArray[tp.Any], kernel: tp.Tuple[int, int], operation: tp.Callable):
    ...
    with pytest.raises(ValueError):
        array_operations.pool_2d_first_two_dimensions(
            arr, kernel, operation=operation)
...
def test_pad_for_hierarchy(arr: npt.NDArray[tp.Any], kernel: tp.Tuple[int, int], fill_value: float):
    ...
    padded = array_operations.pad_for_hierarchy(
        arr, kernel, fill_value=fill_value)
...
def test_compute_max_levels(array, kernel):
    ...
    max_level_x, max_level_y = array_operations.compute_levels(
        array.shape, kernel)
...
def test_hierarchical_pooling(array: npt.NDArray[tp.Any], kernel: tp.Tuple[int, int], operation: tp.Callable):
    ...
    assert len(
        outputs) > 0, "hierarchical_pooling returned an empty list; at least one level of pooling is expected"
...
def test_get_window_from_cell(array: npt.NDArray[tp.Any], kernel: tp.Tuple[int, int], fill_value: float, operation: tp.Callable):
    ...
    window = array_operations.get_window_from_cell(
        outputs, level, cell, kernel)
    ...
    assert np.allclose(
        window, expected_window), "Values in window do not match expected block in previous level"
