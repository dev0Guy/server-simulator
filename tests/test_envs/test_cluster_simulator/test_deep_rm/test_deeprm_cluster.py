from src.envs.cluster_simulator.deep_rm.jobs import DeepRMJobsConvertor
from src.envs.cluster_simulator.deep_rm.machines import DeepRMMachinesConvertor
...
def test_float_cluster_creation_2(
    cluster: DeepRMCluster,
    machine_convertor: DeepRMMachinesConvertor,
    jobs_convertor: DeepRMJobsConvertor,
):
    initialize_machines_usage = machine_convertor.to_representation(cluster._machines)
    initialize_jobs_usage, initialize_jobs_status, initialize_job_arrivals_time = (
...
def test_reproducibility_2(
    params: dict, seed: int, jobs_convertor: DeepRMJobsConvertor
):
    ...
    initialize_jobs_1_usage, initialize_job_1_status, initialize_job_1_arrivals_time = (
        jobs_convertor.to_representation(cluster1._jobs)
    )
    initialize_jobs_2_usage, initialize_job_2_status, initialize_job_2_arrivals_time = (
    ...
    np.testing.assert_array_equal(
        initialize_job_1_arrivals_time, initialize_job_2_arrivals_time
    )
...
def test_different_between_seeds_2(
    params: dict, seed1: int, seed2: int, jobs_convertor: DeepRMJobsConvertor
):
    assume(seed1 != seed2)
    ...
    initialize_jobs_1_usage, initialize_job_1_status, initialize_job_1_arrivals_time = (
        jobs_convertor.to_representation(cluster_1._jobs)
    )
    initialize_jobs_2_usage, initialize_job_2_status, initialize_job_2_arrivals_time = (
        jobs_convertor.to_representation(cluster_2._jobs)
    )
    ...
    assert not np.array_equal(initialize_job_1_status, initialize_job_2_status), (
        "Different seeds should produce different job matrices"
    )
...
def test_job_status_change_to_pending_when_arrival_time_equal_to_current_tick_1(
    cluster: DeepRMCluster, j_idx: int
) -> None:
...
def test_select_single_job_and_run_until_ticks_equal_to_job_length_1(
    cluster: DeepRMCluster,
    m_idx: int,
    j_idx: int,
) -> None:
...
@pytest.mark.usefixtures("machine_convertor", "jobs_convertor")
@given(cluster=DeepRMStrategies.creation())
def test_float_cluster_creation_1(
    cluster: DeepRMCluster,
    jobs_convertor: DeepRMJobsConvertor,
    machine_convertor: DeepRMMachinesConvertor,
):
    ...
    assert initialize_machines_usage.shape[0] == cluster.n_machines
    assert (
        initialize_jobs_usage.shape[0]
        == initialize_jobs_status.shape[0]
        == initialize_jobs_arrival_time.shape[0]
        == cluster.n_jobs
    )
    assert np.all(initialize_machines_usage == 1.0)
    assert np.all(initialize_jobs_usage >= 0.0)
...
def test_different_between_seeds_1(
    params: dict, seed1: int, seed2: int, jobs_convertor: DeepRMJobsConvertor
):
    assume(seed1 != seed2)
    ...
    assert not np.array_equal(initialize_jobs_1_status, initialize_jobs_1_status), (
        "Different seeds should produce different job matrices"
    )
    assert not np.array_equal(
        initialize_jobs_1_arrival_time, initialize_jobs_2_arrival_time
...
@settings(suppress_health_check=[HealthCheck.filter_too_much, HealthCheck.too_slow])
@given(cluster=DeepRMStrategies.creation(), j_idx=st.integers(0))
def test_job_status_change_to_pending_when_arrival_time_equal_to_current_tick_2(
    cluster: DeepRMCluster, j_idx: int
) -> None:
    assume(j_idx < cluster.n_jobs)
    job = cluster._jobs[j_idx]
...
def test_select_single_job_and_run_until_ticks_equal_to_job_length_2(
    cluster: DeepRMCluster,
    m_idx: int,
    j_idx: int,
) -> None:
...
@given(cluster=DeepRMStrategies.creation())
def test_cluster_run_with_random_scheduler_until_completion_2(
    cluster: DeepRMCluster,
) -> None:
    scheduler = RandomScheduler(cluster.is_allocation_possible)
